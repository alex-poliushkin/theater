# Reference

Reference pages record exact contracts.

## Authoring Syntax

- [Theater DSL](theater-dsl/index.md)
- [Theater DSL core syntax](theater-dsl/core-syntax.md)
- [YAML](yaml/index.md)
- [YAML stage schema](yaml/stage-schema.md)

## CLI, Outputs, And Extension APIs

- [CLI](cli/index.md)
- [Output formats](outputs/index.md)
- [Reports](reports.md)
- [Scenario logs](logs.md)
- [Plugins](plugins/index.md)
- [Editor tooling](editor-tooling.md)
- [Go API](go-api.md)

## Capabilities

- [Capabilities](capabilities/index.md)
- [Actions](capabilities/actions.md)
- [HTTP capabilities](capabilities/http.md)
- [Inventories](capabilities/inventories.md)
- [Transforms and YAML decorators](capabilities/decorators.md)
- [Generators](capabilities/generators.md)
- [Matchers](capabilities/matchers.md)
- [State capabilities](capabilities/state.md)

## Selectors And Expectations

- [Selectors](selectors.md)
- [Expectations](expectations.md)

## Ratified Future Contracts

- [Cleanup hooks](cleanup-hooks.md)

Current contract entrypoints:

- [YAML reference](yaml/index.md)
- [Theater DSL reference](theater-dsl/index.md)
- `go run ./cmd/theater help`
- `go run ./cmd/theater explain`
- `report/`
- `plugin/`
- `thtr/`
- `yaml/`
