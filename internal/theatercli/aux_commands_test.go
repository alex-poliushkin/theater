	...
	tests := []struct {
		...
		},
		{
			...
			want: []string{
				...
				"Related:",
				"theater report render --input build/run.json --format summary-md",
				"theater help exit-codes",
			},
		},
		{
...
func TestCompleteCommandUsesMetadata(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "root requirements", args: []string{commandComplete, "re"}, want: []string{commandRequirements, commandReport}},
		{name: "help targets", args: []string{commandComplete, commandHelp, ""}, want: []string{commandInit, commandRun, commandValidate, commandExplain, commandDoctor, commandLibraries, commandPlugins, commandRequirements, commandReport, "environment", "exit-codes", "formats", "debug-selectors", "compatibility", "migration"}},
		...
		{name: "requirements", args: []string{commandComplete, commandRequirements, ""}, want: []string{commandRequirementsInspect}},
		{name: "requirements inspect file flags", args: []string{commandComplete, commandRequirements, commandRequirementsInspect, "--f"}, want: []string{"--file", "--format"}},
		{name: "requirements inspect plugin flags", args: []string{commandComplete, commandRequirements, commandRequirementsInspect, "--p"}, want: []string{"--plugins-config", "--plugins-lock"}},
		{name: "requirements inspect check flag", args: []string{commandComplete, commandRequirements, commandRequirementsInspect, "--c"}, want: []string{"--check-env"}},
		...
		{name: "run summary sidecar flag", args: []string{commandComplete, commandRun, "--s"}, want: []string{"--summary-output"}},
...
func TestCompleteCommandSuggestsStageAndPluginPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "requirements inspect positional stage path",
			args:   []string{commandComplete, commandRequirements, commandRequirementsInspect, filepath.Join(tempDir, "sce")},
			want:   []string{stageYAMLPath, stageTHTRPath},
			absent: []string{pluginConfigPath, notesPath, unsafeStagePath},
		},
		{
			name:   "requirements inspect file flag stage path",
			args:   []string{commandComplete, commandRequirements, commandRequirementsInspect, "--file", filepath.Join(tempDir, "sce")},
			want:   []string{stageYAMLPath, stageTHTRPath},
			absent: []string{pluginConfigPath, notesPath, unsafeStagePath},
		},
		{
			name:   "plugins inspect does not suggest positional stage path",
			args:   []string{commandComplete, commandPlugins, commandPluginsInspect, filepath.Join(tempDir, "sce")},
			absent: []string{stageYAMLPath, stageTHTRPath},
		},
		{
